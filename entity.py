...
class Entity:
    ...
    def move_towards(self, target_x, target_y, game_map, entities):
        ...
        if not (game_map.is_blocked(self.x + dx, self.y + dy) or get_blocked_entities_at_location(entities, self.x + dx, self.y + dy)):
            self.move(dx, dy)
    ...
    def move_astar(self, target, entities, game_map):
        ...
        if not libtcod.path_is_empty(my_path) and libtcod.path_size(my_path) < 25:
            ...
            if x or y:
                ...
                # Keep the old move function as a backpup so that if there are no paths (for example another monster blocks a corridor)
